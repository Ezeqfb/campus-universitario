        ...
        <div class="seccion">
            ...
            if(isset($_GET['id']) && filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
                ...
                if (isset($_POST['submit'])) {
                    // Preparar la consulta para actualizar el nombre del curso
                    $nuevo_nombre = $_POST['nombre'];
                    $sql = "UPDATE cursos SET nombre = ? WHERE id_curso = ?";
                    $stmt = $conexion->prepare($sql);
                    $stmt->bind_param("si", $nuevo_nombre, $id_curso);

                    // Ejecutar la consulta
                    if ($stmt->execute()) {
                        // Mostrar mensaje de éxito si se modificó el curso correctamente
                        echo "<div class='alert alert-success' role='alert'>El curso se modificó con éxito.</div>";
                        echo "<a class='btn btn-primary' href='admin_cursos.php'>Regresar a la lista de cursos</a>";
                    } else {
                        // Mostrar mensaje de error si ocurrió un problema al modificar el curso
                        echo "<div class='alert alert-danger' role='alert'>Error al modificar el curso: " . $stmt->error . "</div>";
                    }

                    // Cerrar la sentencia preparada
                    $stmt->close();
                } else {
                    // Mostrar el formulario para editar el nombre del curso
                    echo "<label for='nombre'>Cambiar nombre:</label>";
                    echo "<form action='' method='post'>";
                    echo "<input class='form-control' placeholder='Nuevo nombre' type='text' name='nombre' required>";
                    echo "<br>";
